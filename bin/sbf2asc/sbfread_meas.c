...
static const SignalType_t Meas3SigIdx2SignalType_Default[MEAS3_SYS_MAX][MEAS3_SIG_MAX] =
{
    ...
    /* GAL */ {SIG_GALE1BC, SIG_GALE5a,  SIG_GALE5b, SIG_GALE6BC, SIG_GALE5,  SIG_RES16_, SIG_LAST,   SIG_LAST,    SIG_LAST, SIG_LAST, SIG_LAST, SIG_LAST, SIG_LAST, SIG_LAST, SIG_LAST, SIG_LAST},
    /* BDS */ {SIG_BDSB1I,  SIG_BDSB2I,  SIG_BDSB3,  SIG_BDSB1C,  SIG_BDSB2a, SIG_BDSB2b, SIG_LAST,   SIG_LAST,    SIG_LAST, SIG_LAST, SIG_LAST, SIG_LAST, SIG_LAST, SIG_LAST, SIG_LAST, SIG_LAST},
    ...
    /* QZS */ {SIG_QZSL1CA, SIG_QZSL2C,  SIG_QZSL5,  SIG_QZSL6,   SIG_QZSL1C, SIG_QZSL1S, SIG_QZSL5S, SIG_QZSL1CB, SIG_LAST, SIG_LAST, SIG_LAST, SIG_LAST, SIG_LAST, SIG_LAST, SIG_LAST, SIG_LAST},
    /* IRN */ {SIG_IRNL5,   SIG_IRNL1,   SIG_IRNS1,  SIG_LAST,    SIG_LAST,   SIG_LAST,   SIG_LAST,   SIG_LAST,    SIG_LAST, SIG_LAST, SIG_LAST, SIG_LAST, SIG_LAST, SIG_LAST, SIG_LAST, SIG_LAST}
};
...
sbfread_Meas3_DecodeMaster(const uint8_t* const buf,
                           ...
                           uint32_t*        SlaveSigMask,          /* slave signal mask decoded in this function */
                           bool             PRRateAvailable,
                           int16_t*         PRRate_64mm_s
#if SSN_FEATURE_SBF_SCRAMBLING
                          ...
                          )
{
    ...
    if ((*buf & 1) == 1)
    {
        ...
        *MasterSigIdx = SigIdxMasterShort;
        ...
        if (LTI3 == 0)
        {
            MeasSet->flags |= (uint8_t)MEASFLAG_HALFCYCLEAMBIGUITY;
        }
    ...
    else if ((*buf & 3) == 0)
    {
        ...
        if (Cont != 0)
        {
            SigMask |= (uint32_t)(BF3 & 0x7f) << 9;
        }

        if (SigMask == 0)
        {
            goto EXIT_INVALIDFORMAT;
        }
...
/*---------------------------------------------------------------------------*/
static uint32_t sbfread_Meas3_DecodeSlave(const uint8_t* const buf,
        SignalType_t     Meas3SigIdx2SignalType[MEAS3_SYS_MAX][MEAS3_SIG_MAX],
        Meas3SatSystem_t SatSys,
        uint32_t         SigIdx,
        int              GLOfn,
...
/*---------------------------------------------------------------------------*/
static int32_t sbfread_Meas3_GetPRRate_mm_s(const Meas3Doppler_1_t* sbfMeas3Doppler,
        uint32_t*       DopplerIdx)
{
...
/*---------------------------------------------------------------------------*/
static void sbfread_Meas3_AddMasterDoppler(MeasSet_t*   MeasSet,
        const Meas3Doppler_1_t* sbfMeas3Doppler,
        double         Wavelength_m,
...
/*---------------------------------------------------------------------------*/
static void sbfread_Meas3_AddSlaveDoppler(MeasSet_t*   MeasSet,
        const MeasSet_t* const MeasSetMaster,
        const Meas3Doppler_1_t* sbfMeas3Doppler,
...
/*---------------------------------------------------------------------------*/
static void sbfread_Meas3_AddPPInfo(MeasSet_t*   MeasSet,
                                    const Meas3PP_1_t* sbfMeas3PP,
                                    uint32_t*      PP1Idx,
                                    uint32_t*      PP2Idx)
{
        ...
        *PP1Idx += 5;
        ...
        if (start2 != 0 && (sbfMeas3PP->Data[start2] & 0xf) == 0)
        {
            ...
            else
            {
                ...
                if (MeasSet->signalType != SIG_GPSL1P && MeasSet->signalType != SIG_GPSL2P)
                {
                    MeasSet->rawCN0_dBHz += 10;
                }
...
/*---------------------------------------------------------------------------*/
static void sbfread_Meas3_AddMPInfo(MeasSet_t*   MeasSet,
                                    const Meas3MP_1_t* sbfMeas3MP,
                                    uint32_t*      MPIdx)
{
    if (sbfMeas3MP != NULL)
    {
        ...
        case 1:
            ...
            if (l < 16)
            {
                MeasSet->CarrierMP_1_512c = (int8_t)l;
            }
            else
            {
                MeasSet->CarrierMP_1_512c = (int8_t)(l - 32);
            }
        ...
        default:
            ...
            {
                MeasSet->MP_mm = (int16_t)((c - 2048) * 10);
            }
            ...
            {
                MeasSet->CarrierMP_1_512c = (int8_t)(l - 256);
            }
...
/*---------------------------------------------------------------------------*/
static MeasChannel_t* sbfread_Meas3_GetMeasChannel(MeasEpoch_t*         MeasEpoch,
        Meas3SatSystem_t     SatSys,
        uint32_t             SatIdx)
{
    uint32_t i;
    ...
    {
        if (MeasEpoch->channelData[i].PRN == (uint8_t)(SVIDBase[SatSys] + SatIdx))
        {
            return &(MeasEpoch->channelData[i]);
        }
    }
...
sbfread_Meas3_DecodeConstellation(const uint8_t* buf,
                                  const Meas3CN0HiRes_1_t* sbfMeas3CN0HiRes,
                                 ...
                                 )
{
    ...
    if (*buf == 0)
    {
        SatDataBuf = RefEpoch->M3SatDataCopy[SatSys];
    }
    else
    {
        SatDataBuf = buf;
    ...
    for (SatIdx = 0; SatIdx < MEAS3_SAT_MAX && SatCnt < NSats; SatIdx++)
    {
        if ((SatMask & (1ULL << SatIdx)) != 0)
        {
            ...
            {
                if (MeasChannel->measSet[AntIdx][SigIdx].signalType >= SIG_LAST)
                {
                    MeasChannel->measSet[AntIdx][SigIdx].flags = 0;
                }
            }
        }
    }
...
)
{
    ...
    for (AntIdx = 0; (int)AntIdx < NR_OF_ANTENNAS; AntIdx++)
    {
        ...
        if (ThisMeas3Ranges != NULL)
        {
            ...
            if ((ThisMeas3Ranges->Misc >> 4) == 0)
            {
                RefEpochInterval_ms =     1;
            }
            else if ((ThisMeas3Ranges->Misc >> 4) == 1)
            {
                RefEpochInterval_ms =   500;
            }
            else if ((ThisMeas3Ranges->Misc >> 4) == 2)
            {
                RefEpochInterval_ms =  1000;
            }
            else if ((ThisMeas3Ranges->Misc >> 4) == 3)
            {
                RefEpochInterval_ms =  2000;
            }
            else if ((ThisMeas3Ranges->Misc >> 4) == 4)
            {
                RefEpochInterval_ms =  5000;
            }
            else if ((ThisMeas3Ranges->Misc >> 4) == 5)
            {
                RefEpochInterval_ms = 10000;
            }
            else if ((ThisMeas3Ranges->Misc >> 4) == 6)
            {
                RefEpochInterval_ms = 15000;
            }
            else if ((ThisMeas3Ranges->Misc >> 4) == 7)
            {
                RefEpochInterval_ms = 30000;
            }
            else if ((ThisMeas3Ranges->Misc >> 4) == 8)
            {
                RefEpochInterval_ms = 60000;
            }
            else if ((ThisMeas3Ranges->Misc >> 4) == 9)
            {
                RefEpochInterval_ms = 120000;
            }
            else
            {
                RefEpochInterval_ms =     1;
            ...
            /* Bit 6 : E6B used */
            if ((ThisMeas3Ranges->CommonFlags & SBF_MEASEPOCH_2_COMMONFLAG_E6BUSED) != 0)
            {
                MeasEpoch->commonFlags |=  COMMONFLAG_E6CENCRYPTED;
            }

            /* Reserved field is a version indication.  This decoder is
               compatible with versions < 32. */
            if (ThisMeas3Ranges->Reserved > 31)
            {
                goto EXIT_INVALIDFORMAT;
            }

            buf = &(ThisMeas3Ranges->Reserved) + 1;
...
/*---------------------------------------------------------------------------*/
static void GetObsFromType1(MeasEpoch_2_t*           MeasEpoch,
    ...
    *AntIdx = Type1SubBlock->Type >> 5;
    ...
    else
    {
        *SignalType = Type1SubBlock->Type & 0x1f;
        ...
        if (*SignalType == 31)
        {
            *SignalType = (Type1SubBlock->ObsInfo >> 3) + 32;
        }
    }
...
/*---------------------------------------------------------------------------*/
static void GetNextObsFromType2(MeasEpoch_2_t*           MeasEpoch,
                                MeasEpochChannelType1_t* Type1SubBlock,
                                SignalType_t*            SignalType,
                                uint32_t*                AntIdx,
                                double*    PR,
 ...
 */
{
    ...
    if (Type2Cnt < Type1SubBlock->N_Type2)
    {
        ...
        *SignalType = Type2SubBlock->Type & 0x1f;
        ...
        if (*SignalType == 31)
        {
            *SignalType = (Type2SubBlock->ObsInfo >> 3) + 32;
        }
...
/*---------------------------------------------------------------------------*/
static uint32_t GetMeasEpochSVID(MeasEpochChannelType1_t* Type1SubBlock)
...
/*---------------------------------------------------------------------------*/
static uint32_t GetMeasEpochRXChannel(MeasEpochChannelType1_t* Type1SubBlock)
...
#if MAX_NR_OF_SIGNALS_PER_SATELLITE < 8
#error "sbfread_meas expects 8 signals per sat"
#endif
...
/*---------------------------------------------------------------------------*/
static bool sbfread_AddMeasSet(MeasChannel_t* trackChan,
                               SignalType_t    SignalType,
                               uint32_t        AntIdx,
                               double          PR_m,
                               ...
                               bool            HCF)
{
        ...
        sigIdx >= 0 && sigIdx < MAX_NR_OF_SIGNALS_PER_SATELLITE)
    {
        ...
        if (LockTime_s == 0xffffffff)
        {
            measSet->PLLTimer_ms = 0;
        }
        else if (LockTime_s == 0)
        {
            measSet->PLLTimer_ms = 10;    /* instead of a hard 0 (==invalid) locktime, we assume 10 ms */
        }
        else
        {
            measSet->PLLTimer_ms = LockTime_s * 1000;
        }
                              ...
                              MeasEpoch_t*         trackMeasEpoch)
{
    ...
    /* Bit 6 : E6B used */
    if ((sbfMeasEpoch->CommonFlags & SBF_MEASEPOCH_2_COMMONFLAG_E6BUSED) != 0)
    {
        trackMeasEpoch->commonFlags |= COMMONFLAG_E6CENCRYPTED;
    }

    Type1SubBlock = GetFirstType1SubBlock(sbfMeasEpoch, &Type1Counter);
    ...
    while (Type1SubBlock)
    {
        ...
        if (trackChan->PRN == 0)
        {
            /* skip, if SVID is not recognized */
            Type1SubBlock = GetNextType1SubBlock(sbfMeasEpoch, Type1SubBlock, &Type1Counter);
            continue;
        }
        ...
        if (agIsGLONASS((unsigned long)trackChan->PRN))
        {
            ...
            {
                /* skip, if frequency number is out of bound */
                Type1SubBlock = GetNextType1SubBlock(sbfMeasEpoch, Type1SubBlock, &Type1Counter);
                continue;
            }
        }
        ...
        /* For some files (e.g. converted from rinex to sbf) the channel nr is unknown.
           ...
           For those files the channel nr is replaced by a fictitious one */
        ...
        if (trackChan->channel >= NR_OF_LOGICALCHANNELS)
        {
            /* skip, if channel number is out of bound */
            Type1SubBlock = GetNextType1SubBlock(sbfMeasEpoch, Type1SubBlock, &Type1Counter);
            continue;
        }
                              ...
                              MeasEpoch_t*     measEpoch)
{
    ...
    uint32_t nrSB = ((sbfMeasExtra->Header.Length / sbfMeasExtra->SBSize - sbfMeasExtra->N) / 256) * 256 + sbfMeasExtra->N;

    /* align the sub-block size to what we expect */
    memcpy(&sbfMeasExtra_Aligned, sbfMeasExtra, sbfMeasExtra->Header.Length);
    AlignSubBlockSize(&sbfMeasExtra_Aligned, (unsigned)nrSB,
    ...
    for (uint32_t i = 0; i < nrSB; i++)
    {
        ...
        if (sigID == 31)
        {
            sigID = (uint8_t)((extraChan->Misc >> 3) + 32);
        }
        ...
        if (chFound)
        {
            ...
            do
            {
                if (measChan->measSet[antNR][j].signalType == sigID)
                {
                    ...
                    {
                        meas->CN0_dBHz += (extraChan->Misc & 0x7) * 0.03125f;
                    }
...
/*! decodes a MeasFullRange_1_t (SBF Block) into an existing
    MeasEpoch_t
 */
static void sbfread_MeasFullRange_Decode(MeasFullRange_1_t*  sbfMeasFullRange,
        MeasEpoch_t*        trackMeasEpoch)
{
    /* Local declarations */
    uint32_t  n;
    uint32_t nrSB = ((sbfMeasFullRange->Header.Length / sbfMeasFullRange->SBLength - sbfMeasFullRange->N) / 256) * 256 + sbfMeasFullRange->N;
    ...
    AlignSubBlockSize(&sbfMeasFullRange_Aligned, (unsigned)nrSB,
...
/*---------------------------------------------------------------------------*/
static bool sbfread_ProcessEpoch(SBFData_t*           SBFData,
                                 MeasEpoch_t*         MeasEpoch,
...
/*---------------------------------------------------------------------------*/
bool sbfread_MeasCollectAndDecode(SBFData_t*           SBFData,
                                  void*                SBFBlock,
                                  MeasEpoch_t*         MeasEpoch,
                                  uint32_t             EnabledMeasTypes)
{
...
#if (NR_OF_ANTENNAS*MEASCOLLECT_NR_OF_MEASBLOCKS > 32)
    /* we have a 32-bit bitfield to keep track of which block we have
       seen in this epoch.  We check here that this is enough.*/
# error MeasEpoch: cannot collect!
#endif
    ...
    case sbfnr_Meas3Ranges_1:
        ...
        if ((int)AntIdx < NR_OF_ANTENNAS)
        {
            ...
            SBFData->MeasCollect_BlocksSeenAtThisEpoch |= (MEASCOLLECT_SEEN_MEAS3RANGES << (AntIdx * MEASCOLLECT_NR_OF_MEASBLOCKS));
        }
    ...
    case sbfnr_Meas3Doppler_1:
        ...
        if ((int)AntIdx < NR_OF_ANTENNAS)
        {
            ...
            SBFData->MeasCollect_BlocksSeenAtThisEpoch |= (MEASCOLLECT_SEEN_MEAS3DOPPLER << (AntIdx * MEASCOLLECT_NR_OF_MEASBLOCKS));
        }
    ...
    case sbfnr_Meas3CN0HiRes_1:
        ...
        if ((int)AntIdx < NR_OF_ANTENNAS)
        {
            ...
            SBFData->MeasCollect_BlocksSeenAtThisEpoch |= (MEASCOLLECT_SEEN_MEAS3CN0HIRES << (AntIdx * MEASCOLLECT_NR_OF_MEASBLOCKS));
        }
    ...
    case sbfnr_Meas3PP_1:
        ...
        if ((int)AntIdx < NR_OF_ANTENNAS)
        {
            ...
            SBFData->MeasCollect_BlocksSeenAtThisEpoch |= (MEASCOLLECT_SEEN_MEAS3PP << (AntIdx * MEASCOLLECT_NR_OF_MEASBLOCKS));
        }
    ...
    case sbfnr_Meas3MP_1:
        ...
        if ((int)AntIdx < NR_OF_ANTENNAS)
        {
            ...
            SBFData->MeasCollect_BlocksSeenAtThisEpoch |= (MEASCOLLECT_SEEN_MEAS3MP << (AntIdx * MEASCOLLECT_NR_OF_MEASBLOCKS));
        }
    ...
    if (SBFData->F != NULL)
    {
        if (SBFData->MeasCollect_BlocksSeenAtThisEpoch != 0)
        {
    ...
    else if (SBFData->MeasCollect_PredictEndOfEpoch &&
             SBFData->MeasCollect_BlocksSeenAtThisEpoch != 0 &&
